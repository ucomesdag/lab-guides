...
#. Run the following commands to reset the password and continue boot:
   ...
   .. code-block:: shell
    ...
    # Check the original SELinux context on /etc/shadow to identify what context to restore
    $ ls -Z /etc/shadow

    # Set the password for the core user
    $ passwd core

    # Manually restore the correct SELinux context on /etc/shadow
    $ chcon system_u:object_r:shadow_t:s0 /etc/shadow

    # Switch to real init system to resume normal boot
    $ exec /usr/sbin/init
...
=================
  ...
  .. code-block:: shell
    ...
    # [... debug pod initialization output omitted ...]
    sh-5.1# chroot /host
    sh-5.1# shutdown -h now
