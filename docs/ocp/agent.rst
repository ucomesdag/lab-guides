...
1. View the list of Operators available to the cluster from OperatorHub:
  ...
  If more than one catalog is installed in a namespace, run the following command to look up the available versions and 
  channels of an Operator from a specific catalog:
...
2. Create the `OperatorGroup`:
  ...
  .. important:: 
    Skip creating the `OperatorGroup` if the operator supports the `AllNamespaces` install mode and you want to use this mode.

  Create an `OperatorGroup` to install the Operator in a specific namespace:
...
3. Create a Subscription to install the Operator from the `OperatorHub`:
  ...
  .. code-block:: yaml
    ...
    metadata:
      ...
      namespace: openshift-operators # use this for AllNamespaces
    spec:
  ...
  See `doc.redhat.com`_ for more Subscription configuration options.

  .. _doc.redhat.com: https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html-single/operators/index#olm-installing-operator-from-software-catalog-using-cli_olm-adding-operators-to-a-cluster
...
1. Check the subscription:
  ...
  If you created an Operator group for SingleNamespace install mode, check the status of the OperatorGroup object by 
